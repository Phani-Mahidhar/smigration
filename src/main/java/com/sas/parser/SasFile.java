...
public class SasFile {
  ...
  public static com.sas.parser.SasParser parseFile(Path filePath) {
    if (filePath == null || !filePath.toFile().exists()) {
      throw new IllegalArgumentException("File does not exist");
    }
    if (!SasFile.isSasFile(filePath)) {
      throw new IllegalArgumentException("File must have a .sas or .inc extension");
    }
    try {
      return parseFileContent(new String(Files.readAllBytes(filePath)));
    } catch (IOException e) {
